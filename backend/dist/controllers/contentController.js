...
export async function getProjectBySlug(req, res) {
    ...
    const doc = await Portfolio.findOne({}).select("projects").lean();
    const project = (doc?.projects || []).find((item) => (item.slug || slugify(item.title)) === slug);
    if (!project)
        return res.status(404).json({ message: "Project not found" });
    return res.json(project);
}
export async function replaceProjects(req, res) {
...
export async function updateProject(req, res) {
    ...
    const index = projects.findIndex((item) => (item.slug || slugify(item.title)) === req.params.slug);
    if (index === -1)
        return res.status(404).json({ message: "Project not found" });
...
export async function deleteProject(req, res) {
    const doc = await getPortfolioDocument();
    const projects = doc.get("projects") || [];
    const next = projects.filter((item) => (item.slug || slugify(item.title)) !== req.params.slug);
...
export async function getBlogBySlug(req, res) {
    const slug = req.params.slug;
    const doc = await Portfolio.findOne({}).select("blogs").lean();
    const blog = (doc?.blogs || []).find((item) => (item.slug || slugify(item.title)) === slug);
    if (!blog)
        return res.status(404).json({ message: "Blog not found" });
    return res.json(blog);
}
export async function replaceBlogs(req, res) {
    const doc = await getPortfolioDocument();
...
export async function updateBlog(req, res) {
    ...
    const index = blogs.findIndex((item) => (item.slug || slugify(item.title)) === req.params.slug);
    if (index === -1)
        return res.status(404).json({ message: "Blog not found" });
...
export async function deleteBlog(req, res) {
    const doc = await getPortfolioDocument();
    const blogs = doc.get("blogs") || [];
    const next = blogs.filter((item) => (item.slug || slugify(item.title)) !== req.params.slug);
